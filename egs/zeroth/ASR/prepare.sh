...
#
#  - $dl_dir/zeroth
#      You can find AUDIO_INFO, and
#      train dev test
#      It will be downloaded from aws with open credential
#
#  - $dl_dir/lm
...
# For now, bpe model is not used for Korean Zeroth model. 
# It is already addressed in LM building status
...
if [ $stage -le 0 ] && [ $stop_stage -ge 0 ]; then
  ...
  # If you have pre-downloaded it to /path/to/zeroth,
...
if [ $stage -le 1 ] && [ $stop_stage -ge 1 ]; then
  ...
  # We assume that you have downloaded the ZerothSpeech corpus
  # to $dl_dir/zeroth
  mkdir -p data/manifests
  lhotse prepare zeroth -j $nj $dl_dir/zeroth $dl_dir/lm/zeroth_morfessor.seg data/manifests
fi
...
#       cat $(find download/zeroth -name *.txt) |\
#         python3 local/filterSentence.py --include False - - |\
#         cut -d " " -f 2- |\
#         sort | uniq > $lang_dir/train.txt
...
if [ $stage -le 7 ] && [ $stop_stage -ge 7 ]; then
  ...
  if [ ! -f data/lm/G_3_gram.fst.txt ]; then
    ...
    [ ! -f $dl_dir/lm/zeroth.lm.tgsmall.arpa ] && \
      gunzip $dl_dir/lm/zeroth.lm.tgsmall.arpa.gz
    ...
    python3 -m kaldilm \
      ...
      --disambig-symbol='#0' \
      --max-order=3 \
      $dl_dir/lm/zeroth.lm.tgsmall.arpa > data/lm/G_3_gram.fst.txt
  fi
  ...
  if [ ! -f data/lm/G_4_gram.fst.txt ]; then
    ...
    python3 -m kaldilm \
      ...
      --disambig-symbol='#0' \
      --max-order=4 \
      $dl_dir/lm/zeroth.lm.tgmed.arpa > data/lm/G_4_gram.fst.txt
  fi
fi
...
# cd data && ln -snf lang_bpe_500 lang_bpe
